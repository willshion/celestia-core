...
func TestVoteSet_2_3MajorityRedux(t *testing.T) {
	...
	blockDAHeader := makeDAHeaderRandom()
	...
	voteProto := &Vote{
		...
		Timestamp:        tmtime.Now(),
		Type:             tmproto.PrevoteType,
		BlockID:          BlockID{blockHash, blockPartSetHeader, blockDAHeader},
	}
	...
	// 71st validator voted for the right BlockHash & BlockPartSetHeader
	{
		...
		require.NoError(t, err)
		blockID, ok = voteSet.TwoThirdsMajority()
		assert.True(t, ok && blockID.Equals(BlockID{blockHash, blockPartSetHeader, blockDAHeader}),
...
func TestVoteSet_Conflicts(t *testing.T) {
	...
	err = voteSet.SetPeerMaj23("peerA", BlockID{blockHash1, PartSetHeader{}, nil})
	require.NoError(t, err)
	...
	err = voteSet.SetPeerMaj23("peerA", BlockID{blockHash2, PartSetHeader{}, nil})
	require.Error(t, err)
	...
	err = voteSet.SetPeerMaj23("peerB", BlockID{blockHash1, PartSetHeader{}, nil})
	require.NoError(t, err)
